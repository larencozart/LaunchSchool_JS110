...
not have to worry about floating point errors. You may also assume
that the arguments are in degrees.
*/

function triangle(angle1, angle2, angle3) {
  const angles = [angle1, angle2, angle3];
  const angleSum = angles.reduce((accum, el) => accum + el, 0);


  if (angleSum !== 180 || !angles.every(angle => angle > 0)) {
    return 'invalid';
  }

  let classification;
  if (angles.some(angle => angle === 90)) { // right
    classification = 'right';
  } else if (angles.every(angle => angle < 90)) { // acute
    classification = 'acute';
  } else if (angles.some(angle => angle > 90)) { // obtuse
